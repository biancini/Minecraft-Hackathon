...
public class EZPlugin extends Plugin implements CommandListener {
	...
	/**************************************************
	 * Convenience helper methods you can call from within our plugins, or
	 * anywhere else as EZ.name
	 **************************************************/
	...
	public static void playSound(Location loc, SoundEffect.Type type, double volume, double pitch) {
		loc.getWorld()
				.playSound(new SoundEffect(type, loc.getX(), loc.getY(), loc.getZ(), (float) volume, (float) pitch));
	}

	public static void playSound(Location loc, SoundEffect.Type type) {
	...
	public static void spawnParticle(Location loc, Particle.Type type) {
		loc.getWorld().spawnParticle(new Particle(loc.getX(), loc.getY(), loc.getZ(), type));
	}
